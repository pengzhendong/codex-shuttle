...
- **版本绑定发布**：每 15 分钟检测官方稳定源码版本，同时永久保留旧版 Release。
...
GitHub Actions 每 15 分钟检查一次 OpenAI 稳定 `rust-vX.Y.Z` 标签，并按顺序补齐每个尚未发布的版本。只有 workspace 测试、两种 Mac CLI、两种 Linux shim 和两种 Linux runtime 烟测全部通过，才会发布绑定版本的 Shuttle Release。这些门禁证明构建兼容；针对具体 Mac 和服务器的端到端检查仍以 `cxs doctor` 为准。构建失败不会生成半成品，并会在下一轮自动重试。
