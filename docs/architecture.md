...
The local bridge supervises that SSH/Yamux transport. If it ends unexpectedly, the same bridge process reconnects with exponential backoff from 250 milliseconds to 5 seconds; a connection that remains stable for 30 seconds resets the backoff. The local compatibility endpoint exists only while the transport is ready, allowing status checks to distinguish a live reconnecting supervisor from a usable connection.
